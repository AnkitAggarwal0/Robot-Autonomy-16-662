...
########### Search ###########
vertices=[] # List of Visited States -- visited in pseudo code
parent=[]
action=[]
cost2come=[]
...
while len(pq)>0:
    ...
    if CheckCondition(vertices[idx], GoalState):
        FoundPath=True
        x=idx
        break
    ...
    for action_idx in range(len(ActionPre)):
        ...
        if not CheckCondition(vertices[idx], ActionPre[action_idx]):
            continue #skip if the action is not applicable

        # Get the next state, use ComputeNextState
        nextState = ComputeNextState(vertices[idx], ActionEff[action_idx])
        ...
        if not CheckVisited(nextState, vertices):
            vertices.append(nextState)
            parent.append(idx)
            action.append(action_idx)
            cost2come.append(cost2come[idx] + 1)

            # Djikstra: cost is the cost to come
            cost = cost2come[-1]
